...
def main():
    ...
    with st.sidebar:    
        ...
        with col3:
            new_lang = st.selectbox("Język", list(lang_mapping3.keys()), key="new_lang_select")
        ...
        for idx, word_data in enumerate(vocabulary):
            ...
            if st.button(f"Usuń {word} -> {translation}", key=f"vocab_{word_data[0]}_{idx}"):
    ...
    with tab1:
        ...
        if st.session_state.translated_text:
            ...
            st.write("Najpierw wykonaj tłumaczenie, aby wygenerować quiz")
